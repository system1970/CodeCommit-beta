...
if len(json_format)!=0:
    for i in range(len(json_format)):
        if json_format[i]["verdict"]=="OK" and json_format[i]["problem"]["name"] not in problems:
            ...
            Solution = str(mydivs).replace('[<pre class="prettyprint lang-py linenums program-source" id="program-source-text" style="padding: 0.5em;">','https://codeforces.com/problemset/problem/'+str(ContestId)+'/'+str(json_format[i]["problem"]["index"])+"\n\n"+"              "+str(json_format[i]["problem"]["name"])+":"+"\n\n").replace("</pre>]","")
